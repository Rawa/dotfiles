...
local freq_cache = awful_util.getdir("cache") .. "/freq_menu"
...
function utils.update_cache_entry_value(cmdline, value)
    ...
    if(hFile ~= nil) then
        for line in hFile:lines() do
            if(starts_with(line, cmdline)) then --Is this the line to modify?
                lines[#lines + 1] = cmdline .. " " .. value --Change old line into new line.
    ...
    hFile = io.open(freq_cache, "w") --write the file.
